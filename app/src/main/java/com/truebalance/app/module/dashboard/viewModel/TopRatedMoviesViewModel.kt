...
@HiltViewModel
class TopRatedMoviesViewModel @Inject constructor( private val repo : MoviesRepository) : BaseViewModel() {
    ...
    private fun getAPiTopRated() = viewModelScope.launch {
        repo.topRatedMoviesApi().collect {
            when (it.status) {
